...
class Evaluator():
    ...
    def evaluate_multiple_recordings(self, process_complete=False, tworounds=False):
        ...
        csv_file=os.path.join(path_dataset,self.args.tester.evaluation.csv_file)
        #read the csv file
        list_recordings=pd.read_csv(csv_file, header=None)
        for i, row in list_recordings.iterrows():
            recording=row[0]
            recording_path=os.path.join(path_dataset, recording)
            ...
            #except:
            #    print("error in recording", recording_path)
            #    continue
    ...
    def evaluate_single_recording(self, recording_path=None, process_complete=False,tworounds=False):
        ...
        if not process_complete:
            ...
            if self.args.tester.evaluation.LTAS_init:
                ...
                print("recording[0].shape", recording.shape)

                max=LTAS_smooth.max()
                correction=1/max #that correction may not be necessary, but I will keep it for now
                LTAS_smooth=LTAS_smooth*correction

                LTAS_y, std=blind_bwe_utils.compute_LTAS(x=recording.clone(), sample_rate=self.args.exp.sample_rate, normalize=0.07)
                LTAS_y_smooth=blind_bwe_utils.smooth_LTAS(LTAS_y, freqs, Noct=3)*correction
        ...
        if not process_complete:
            ...
            rid=False
            outputs=self.sampler.predict_blind_bwe(y, rid=rid, x_init=x_seg_init if self.args.tester.evaluation.LTAS_init else None)
