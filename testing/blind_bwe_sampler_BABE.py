...
class LPFOperator():
    def __init__(self, args, device) -> None:
        ...
        self.params_fc=torch.Tensor(self.args.tester.blind_bwe.initial_conditions.fc).to(device)
        #self.params_fc=torch.nn.Parameter(self.params_fc)

        self.params_A=torch.Tensor(self.args.tester.blind_bwe.initial_conditions.A).to(device)
        #self.params_A=torch.nn.Parameter(self.params_A)


        self.freqs=torch.fft.rfftfreq(self.args.tester.blind_bwe.NFFT, d=1/self.args.exp.sample_rate).to(self.device) #self.degradation=lambda x, params: self.apply_filter_fcA(x,  params)


        self.params=[self.params_fc, self.params_A]
    ...
    def assign_params(self, params):
        ...
        self.params_fc=params[0]
        self.params_A=params[1]
        self.params=[self.params_fc,self.params_A]
...
class BlindSampler():
    ...
    def __init__(self, model,  diff_params, args, rid=False):
        ...
        if self.args.tester.posterior_sampling.rec_distance_operator=="l2":
            def loss_fn_op(x_hat, x):
                diff=x_hat-x
                #if self.args.tester.filter_out_cqt_DC_Nyq:
                #    diff=self.model.CQTransform.apply_hpf_DC(diff)
                return (diff**2).mean()/2

            self.rec_distance_operator=lambda x_hat, x: loss_fn_op(x_hat, x)

        elif self.args.tester.posterior_sampling.rec_distance_operator=="l2_hp":
            filter=FIRFilter( filter_type="hp", coef=0.85, fs=self.args.exp.sample_rate, ntaps=3, plot=False).to(self.model.device)
            #fir = torch.nn.Conv1d(1, 1, kernel_size=3, bias=False, padding=1).to(self.model.device)
            #fir.weight.requires_grad = False
            #fir.weight.data = torch.tensor([1, -0.85, 0]).view(1, 1, -1).to(self.model.device)
            def loss_fn_op(x_hat, x):
                #print("in loss")
                #print(x_hat.shape)
                #x_hat=fir.forward(x_hat)
                #x=fir.forward(x)
                x_hat, x=filter.forward(x_hat, x)
                #print(x_hat.shape)
                diff=x_hat-x
                if self.args.tester.filter_out_cqt_DC_Nyq:
                    diff=self.model.CQTransform.apply_hpf_DC(diff)
        ...
        elif self.args.tester.posterior_sampling.rec_distance_operator=="multiresSTFT":
            loss_fn_op = auraloss.freq.MultiResolutionSTFTLoss(
                fft_sizes=[512, 1024, 4092],
                hop_sizes=[128, 256, 1024],
                win_lengths=[512, 1024, 4092],
                sample_rate=self.args.exp.sample_rate,
            )
            self.rec_distance_operator=lambda x_hat, x: loss_fn_op(x_hat, x)
        elif self.args.tester.posterior_sampling.rec_distance_operator=="magSTFT":
            def loss_fn_op(x_hat, x):

                X_hat=blind_bwe_utils.apply_stft(x_hat, 2048)
                #normalize by the window energy
                X_hat/=2048**0.5
                X=blind_bwe_utils.apply_stft(x, 2048)
                X/=2048**0.5
                ...
                X_hat=X_hat*torch.log2(1+freqs.unsqueeze(-1).expand(X_hat.shape))

                return ((X.abs()-X_hat.abs())**2).mean()/2
            #loss_fn = auraloss.freq.STFTLoss(
            #)
            self.rec_distance_operator=lambda x_hat, x: loss_fn_op(x_hat, x)
        elif self.args.tester.posterior_sampling.rec_distance_operator=="magSTFT_hp":
            ...
            #)
            self.rec_distance_operator=lambda x_hat, x: loss_fn_op(x_hat, x)
        elif self.args.tester.posterior_sampling.rec_distance_operator=="magSTFT_freq_weighted":
            def loss_fn_op(x_hat, x):
                X_hat=blind_bwe_utils.apply_stft(x_hat, 2048)
                #normalize by the window energy
                X_hat/=2048**0.5
        ...
        if self.args.tester.posterior_sampling.rec_distance=="l2_mean":
        ...
        elif self.args.tester.posterior_sampling.rec_distance=="l2_sum":
            def loss_fn_rec(x_hat, x):
                diff=x_hat-x
                #if self.args.tester.filter_out_cqt_DC_Nyq:
                #    diff=self.model.CQTransform.apply_hpf_DC(diff)
            ...
            self.rec_distance=lambda x_hat, x: loss_fn_rec(x_hat, x)
        elif self.args.tester.posterior_sampling.rec_distance=="l2_hp":
            filter=FIRFilter( filter_type="hp", coef=0.85, fs=self.args.exp.sample_rate, ntaps=3, plot=False).to(self.model.device)
            #fir = torch.nn.Conv1d(1, 1, kernel_size=3, bias=False, padding=1).to(self.model.device)
            #fir.weight.requires_grad = False
            #fir.weight.data = torch.tensor([1, -0.85, 0]).view(1, 1, -1).to(self.model.device)
        ...
        elif self.args.tester.posterior_sampling.rec_distance=="multiresSTFT":
            loss_fn_rec = auraloss.freq.MultiResolutionSTFTLoss(
                fft_sizes=[512, 1024, 4092],
                hop_sizes=[128, 256, 1024],
                win_lengths=[512, 1024, 4092],
                sample_rate=self.args.exp.sample_rate,
            ...
            self.rec_distance=lambda x_hat, x: loss_fn_rec(x_hat, x)
        elif self.args.tester.posterior_sampling.rec_distance=="magSTFT":
            def loss_fn_rec(x_hat, x):
                X_hat=blind_bwe_utils.apply_stft(x_hat, 2048)
                #normalize by the window energy
                X_hat/=2048**0.5
            ...
            #)
            self.rec_distance=lambda x_hat, x: loss_fn_rec(x_hat, x)
        elif self.args.tester.posterior_sampling.rec_distance=="magSTFT_sum":
            def loss_fn_rec(x_hat, x):
                X_hat=blind_bwe_utils.apply_stft(x_hat, 2048)
                #normalize by the window energy
                X_hat/=2048**0.5
                ...
                return ((X.abs()-X_hat.abs())**2).sum()/2
            #loss_fn = auraloss.freq.STFTLoss(
            #)
            self.rec_distance=lambda x_hat, x: loss_fn_rec(x_hat, x)
        elif self.args.tester.posterior_sampling.rec_distance=="magSTFT_freq_weighted":
            def loss_fn_rec(x_hat, x):
                X_hat=blind_bwe_utils.apply_stft(x_hat, 2048)
                #normalize by the window energy
                X_hat/=2048**0.5
    ...
    def predict_bwe_AR(
        ...
        return self.predict_conditional(y, rid,  compute_sweep)
    ...
    def predict_bwe(
        ...
        return self.predict_conditional(ylpf, rid,  compute_sweep)
    ...
    def predict_conditional(
        ...
        return self.predict(y, rid, compute_sweep, blind=False)
    ...
    def fit_params(self, denoised_estimate, y):
        ...
        for i in tqdm(range(self.args.tester.blind_bwe.optimization.max_iter)):
            ...
            #filt_params=torch.stack(self.operator.params)
            grad=torch.autograd.grad(loss,self.operator.params,create_graph=True)
            #print("grad,", grad)
            t=self.mu
            #newparams=filter_params-t.unsqueeze(1)*grad[0]
            #newparams.detach_()
            #print(t)
            self.operator.params[0]=self.operator.params[0]-t[0]*grad[0]
            self.operator.params[1]=self.operator.params[1]-t[1]*grad[1]
            for j in range(len(self.operator.params)):
                self.operator.params[j].detach_()
            #self.operator.params[1]=newparams[1]
    ...
    def predict_blind_bwe_AR(
        ...
        y=y.unsqueeze(0)
        return self.predict(y, rid, compute_sweep, blind=True)
    ...
    def predict(
        ...
        for i in tqdm(range(0, self.nb_steps, 1)):
            ...
            if self.args.tester.wandb.use:
                ...
                if blind:
                    #Plot filter
                    fig=blind_bwe_utils.plot_single_filter_BABE(self.operator.params, self.operator.freqs)
